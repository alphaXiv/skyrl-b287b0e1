...
# ─── SDPO sync K=0, G=4 sibling-rollout hint, tau-retail (no-user) ───────
# Model: Qwen3-4B. WandB: sdpo-tau-retail-glm.
# Hint = successful sibling rollout text (from G=4 groups).
...
RUN_NAME="sdpo_sync_k0_g4_sibling_seed0"
...
python -m skyrl.train.entrypoints.main_sdpo \
  ...
  trainer.algorithm.sdpo.success_reward_threshold=0.5 \
...
# SDPO Sync K=0, G=4 Sibling-Rollout Hint — Tau-Retail (Qwen3-4B)

## Config
- Algorithm: SDPO (reverse KL, sampled-token estimator)
- Staleness: K=0 (sync, colocated)
- Group size: G=4 (hint = successful sibling rollout text)
- IS correction: disabled (use_is=false)
- Model: Qwen/Qwen3-4B
- Steps: 40
- Batch size: 16
- Hint: decoded text of successful sibling rollout (reward >= 0.5)
- Teacher: frozen ref model on reprompted input (hint in user message)
